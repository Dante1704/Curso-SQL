...
--selecciona todos los registros de la tabla y decime cuantos son
  select count(*) as "Cant Registros" from empleados; 

--consulta por la cantidad de sueldos no nulos en el puesto de asistente
  select count(sueldo) as "Cant Asists Con Salario" from empleados
  where puesto = 'Asistente';
  
-- Suma Agrupacion, sum()

-- consulta por la suma de todos los sueldos en puesto de gerente
select sum(sueldo) as "Nomina" from empleados
...
--consulta por el promedio de sueldo excluyendo los repetidos
--este promedio es distinto del promedio total porque con el distinct excluyo los repetidos
select avg(distinct sueldo) as "Sueldo promedio sin repetir sueldos" from empleados;



select 
count(*) as registros, 
avg(sueldo) as promedio_sueldos, 
sum(cant_hijos) as total_hijos 
from empleados;
